...
class Vent {
	...
	public List<Coord> getCloudCoverage() {
		...
		if (start.getX() < end.getX() && start.getY() < end.getY()) { // NE
			int span = end.getX() - start.getX();
			for (int i = 0; i <= span; i++) {
				retList.add(new Coord(start.getX() + i, start.getY() + i));
			}
		} else if (start.getX() < end.getX() && start.getY() > end.getY()) { // SE
			int span = end.getX() - start.getX();
			for (int i = 0; i <= span; i++) {
				retList.add(new Coord(start.getX() + i, start.getY() - i));
			}
		} else if (start.getX() > end.getX() && start.getY() > end.getY()) { // SW
			int span = start.getX() - end.getX();
			for (int i = 0; i <= span; i++) {
				retList.add(new Coord(start.getX() - i, start.getY() - i));
			}
		} else { // NW
