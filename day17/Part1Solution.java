...
public class Part1Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int x = xVelocity; x <= ta.getMaxX(); x++) { // If the velocity is > maxX it will overshoot on the first move
            for (int y = yVelocity; y >= ta.getMaxY(); y--) {
                Probe probe = new Probe(x, y);
                Coord curPoint = probe.getCoords();
                int maxYForCurVelocity = curPoint.getY();
                while (!ta.isPastTarget(curPoint)) {
                    probe.move();
                    curPoint = probe.getCoords();
                    if (curPoint.getY() > maxYForCurVelocity) {
                        maxYForCurVelocity = curPoint.getY();
                    }
                    if (ta.isOnTarget(curPoint)) {
                        System.out.print("(" + x + ", " + y + ")");
                        if (maxYForCurVelocity > maxY) {
                            maxY = maxYForCurVelocity;
                            break;
                        }
                    }
                }
        ...
        System.out.println("Solution is: " + maxY);
    }
